function [mssim, ssim_map] = Qa_SSIM(img1, img2, K, window, L)
%% test
%{
取值范围：
ssim = Qa_SSIM(img1,img1)
ssim =
     1

ssim = Qa_SSIM(img1,img2) 'g2.bmp' '11.bmp'
ssim =
    0.1893

img1 = imread('ssim\w.bmp');
img2 = imread('ssim\b.bmp');  %'j11.jpg'
ssim = Qa_SSIM(img1,img2)
ssim =
  9.9990e-005
%}
%% 读入图片
...
if (size(img1) ~= size(img2)) %要求size一样
   mssim = -Inf;
   ssim_map = -Inf;
   return;
end
