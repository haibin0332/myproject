...
%}
function [hashVector] = mbe_Fridrich_HVSWeighted_reciprocal(imagefile,param)
%% test inputs
% nargin = 1;
...
% 	II(rectVertexX(i):(rectVertexX(i) + rectBlockNx(i) * 8 - 1),rectVertexY(i):(rectVertexY(i) + rectBlockNy(i) * 8 - 1)) = 255;
	...
	for k = 1:N
		H(i, k) = sum(sum(randRectBlocks.* (1./randBlockMask).* P(:,:,k))); % 针对 001n-JPEG-70.jpg 在i=3,k = 2时可以看到复数的产生。
	end
end
% 	hold on; imshow(II);
%% 二值化
