...
%}
function [hashVector] = mbe_Fridrich_HVSOnly(imagefile,param)
%% test inputs
% nargin = 1;
...
if method == 1	%随机分块
	rand('seed',key);
	rectVertexX = ceil((sizeOfImage/8 - 1) * rand(M,1)) * 8 + 1;		% 分块的顶点坐标：（rectVertexX, rectVertexY), 起点一定与DCT块对齐
	rectVertexY = ceil((sizeOfImage/8 - 1) * rand(M,1)) * 8 + 1;		% 分块的顶点是随机的，由密钥控制
	for i = 1: M
		rectBlockNx(i) = ceil(ceil((sizeOfImage - rectVertexX(i))/8) * rand(1));	% 每个块占多少列（以8为单位，因为要与DCT块对齐）
		rectBlockNy(i) = ceil(ceil((sizeOfImage - rectVertexY(i))/8) * rand(1));	% 每个块占多少行
		if rectBlockNx(i) == 0
			rectBlockNx(i) = 1;
		end
		if rectBlockNy(i) == 0
...
% 	rectVertexY = zeros([1, M*M]);
	...
	rectBlockNx(1:M*M) = sizeOfImage/M/8;
...
for i = 1:M
	...
	P = zeros([rectBlockNx(i) * 8, rectBlockNy(i) * 8, N]);
	P = rand([rectBlockNx(i) * 8, rectBlockNy(i) * 8, N]);
	...
	% 取得这个分块的矩阵
	randRectBlocks = I(rectVertexX(i):(rectVertexX(i) + rectBlockNx(i) * 8 - 1),rectVertexY(i):(rectVertexY(i) + rectBlockNy(i) * 8 - 1));
    randBlockMask = contrastThreshold(rectVertexX(i):(rectVertexX(i)+rectBlockNx(i) * 8 - 1),rectVertexY(i):(rectVertexY(i) + rectBlockNy(i) * 8 - 1));

	%{
	% for test random blocking
% 	II = I;
	II(rectVertexX(i), rectVertexY(i):(rectVertexY(i) + rectBlockNy(i) * 8 - 1)) = 255;
	II(rectVertexX(i):(rectVertexX(i) + rectBlockNx(i) * 8 - 1), rectVertexY(i)) = 255;
	II((rectVertexX(i) + rectBlockNx(i) * 8 - 1), rectVertexY(i):(rectVertexY(i) + rectBlockNy(i) * 8 - 1)) = 255;
	II(rectVertexX(i):(rectVertexX(i) + rectBlockNx(i) * 8 - 1), (rectVertexY(i) + rectBlockNy(i) * 8 - 1)) = 255;
% 	II(rectVertexX(i):(rectVertexX(i) + rectBlockNx(i) * 8 - 1),rectVertexY(i):(rectVertexY(i) + rectBlockNy(i) * 8 - 1)) = 255;
	imshow(II);
	%}
	
	for k = 1:N
		H(i, k) = sum(sum(randBlockMask.* P(:,:,k))); % 针对 001n-JPEG-70.jpg 在i=3,k = 2时可以看到复数的产生。
	end
end
