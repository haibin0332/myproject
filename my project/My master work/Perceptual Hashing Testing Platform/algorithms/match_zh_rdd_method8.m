...
%% get inputs
    if nargin == 2
        ...
        numdiffTimesGlobal = 8; % 这个影响差分的步长
        imageQueried =  'F:\imDatabase\TestImages\Substitute with Signal Block\baboon-SubSignal-0.046875.bmp';  % 'F:\imDatabase\TestImages\Gaussian Noise(mean)\baboon-GNoiseMean-0.2.bmp'; % query的图像，用于标注位置 
    ...
    Tlocal = 0.3;   % 这个阈值是通过试验确定的，用于区分local是否变化的
...
%         globalhash((i-1)*numrects+1:(i*numrects)) = meanBlocks(iA) >= meanBlocks(iB);
        ...
        for j = 1:numrects
                %% 处理被怀疑的块，match，标注
            if iS(j) > 1 % 这里相当于是个阈值。即如果只被怀疑一次，这样的块不处理。
